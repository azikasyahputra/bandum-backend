import { Link } from "@inertiajs/react";
import { jsx } from "react/jsx-runtime";
//#region resources/js/Components/Pagination.jsx
function filterPaginationLinks(links) {
	if (!links || links.length < 3) return links;
	const prev = links[0];
	const next = links[links.length - 1];
	const pages = links.slice(1, -1);
	const firstPage = pages[0];
	const lastPage = pages[pages.length - 1];
	const first = {
		url: firstPage.url,
		label: "« First",
		active: false
	};
	const last = {
		url: lastPage.url,
		label: "Last »",
		active: false
	};
	const activeIdx = pages.findIndex((l) => l.active);
	let start = Math.max(0, activeIdx - 2);
	let end = Math.min(pages.length, start + 5);
	if (end - start < 5) start = Math.max(0, end - 5);
	return [
		first,
		prev,
		...pages.slice(start, end),
		next,
		last
	];
}
function Pagination({ items }) {
	if (!items?.data?.length || !items?.links) return null;
	return /* @__PURE__ */ jsx("div", {
		className: "d-flex justify-content-center mt-10 pagination-wrapper",
		children: /* @__PURE__ */ jsx("nav", { children: /* @__PURE__ */ jsx("ul", {
			className: "pagination mb-0 pagination-list",
			children: filterPaginationLinks(items.links).map((link, i) => /* @__PURE__ */ jsx("li", {
				style: { margin: 0 },
				children: link.url ? /* @__PURE__ */ jsx(Link, {
					href: link.url,
					className: `pagination-link${link.active ? " pagination-link--active" : ""}`,
					dangerouslySetInnerHTML: { __html: link.label }
				}) : /* @__PURE__ */ jsx("span", {
					className: "pagination-link pagination-link--disabled",
					dangerouslySetInnerHTML: { __html: link.label }
				})
			}, i))
		}) })
	});
}
//#endregion
export { Pagination as t };
